		...
		class SMATest(Algorithm):
			...
			def handle_data(self, dt, symbols, keys, data):
				...
				for symbol in symbols:
					...
					if shortsma_yesterday and px_yesterday:
						if shortsma_yesterday < px_yesterday and shortsma_today > px:
							if self.cash > 0:
								self.oms.add(Transaction(symbol, dt, px, self.cash / px))
								self.cash = 0
						elif shortsma_yesterday > px_yesterday and shortsma_today < px:
